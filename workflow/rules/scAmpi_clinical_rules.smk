...
def getGeneSetHeatmapFiles(wildcards):
    ...
    return expand(
        "results/gene_set_enrichment/{sample}.{i}.enrichedGeneSets.txt",
        i=glob_wildcards(
...
def getGeneSetHeatmapFilesMalignant(wildcards):
    checkpoint_output = checkpoints.diff_exp_analysis.get(**wildcards).output[0]
    return expand(
        "results/gene_set_enrichment/vs_other_malignant/{sample}.DEmalignant.{i}.enrichedGeneSets.txt",
        i=glob_wildcards(
            os.path.join(
                checkpoint_output,
...
def get_parse_for_minSetCover_input(wildcards):
    checkpoint_output = checkpoints.diff_exp_analysis.get(**wildcards).output[0]
    return expand(
        "results/clinical_trials/{sample}.{i}.dgidb.txt.CompleteTable.ClinicalTrials.{{type}}.txt",
        i=glob_wildcards(
            os.path.join(checkpoint_output, "{sample,[^/]+}.{i,[^/]+}.DEgenes.tsv")
        ).i,
        sample=wildcards.sample,
...
rule parse_for_minSetCover:
    ...
    params:
        ...
        colName_DGIDB_score=config["tools"]["parse_for_minSetCover"][
            "colName_DGIDB_score"
        ],
...
rule get_full_druglist_to_subclones:
    ...
    params:
        drugList=config["resources"]["drugList"],
        custom_script=workflow.source_path(
            "../scripts/get_full_druglist_to_subclones_assignm.py"
        ),
    conda:
...
def getCivicQueryResults(wildcards):
    checkpoint_output = checkpoints.diff_exp_analysis.get(**wildcards).output[0]
    return expand(
        "results/query_civic/{sample}.{i}.clinicalAnnotation.civic.txt",
        sample=wildcards.sample,
        i=glob_wildcards(
            os.path.join(checkpoint_output, "{sample,[^/]+}.{i,[^/]+}.DEgenes.tsv")
        ).i,
...
rule plot_drug_prediction:
    ...
    params:
        ...
        custom_script=workflow.source_path(
            "../scripts/plot_drug_prediction_on_clones.R"
        ),
    conda:
