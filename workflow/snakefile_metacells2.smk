...
# make sure seacell specific rules are applied if sample wildcard is followed by _metacells2 in the file names
ruleorder: sctransform_preprocessing_filtered_metacells2 > sctransform_preprocessing
...
rule scAmpi_metacells2:
    input:
        ...
        expand(
            "results/plotting/{sample}_metacells2.celltype_barplot.png",
            sample=sample_ids,
        ),
        expand(
            "workflow/report/rules/metacells2/{sample}_celltyping_summary.rst",
            sample=sample_ids,
        ),
