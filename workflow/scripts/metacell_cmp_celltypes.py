...
def main(celltype_file, metacelltype_file, assignment_file,
    ...
    metacell_counts.to_csv(
        f"{out_dir}/{out_prefix}_celltype_counts.tsv", sep='\t', index=False)
...
def get_subcell_dict(celltype_config_file):
    celltype_config = pd.read_csv(
        celltype_config_file, sep='\t').set_index('Major')
    ...
    # simpler version:
    celltype_dict = {m.split('_', 1)[0]: [s.split('_', 1)[0] for s in sl.split(
        ',')] for m, sl in celltype_config['Subtype'].items()}
    subcelltype_dict = {v: k for k, vl in celltype_dict.items()
                        for v in vl if v != 'none'}
...
def get_report(metacell_counts):
    ...
    pure_disagree = metacell_counts.query(
        "purity>.8 and metacell_type != dominant")
    groups = ["B.cell", "T.cell", "Macrophage", "Melanoma"]
    disagree_idx = [idx for idx, row in pure_disagree.iterrows() if not any(
        gr in row.metacell_type and gr in row.dominant for gr in groups)]
...
def get_metacell_counts(celltype_file, metacelltype_file, assignment_file, subcell_dict=None):
    ...
    * number of cells for individual celltypes
    ...
    metacelltypes_dict = dict(
        metacelltypes_df.set_index('barcodes')['celltype_final'])
    metacell_assignment_df = pd.read_csv(assignment_file, sep='\t')
    # Merge the cell type info with the metacell assignment info
    merged_df = pd.merge(metacell_assignment_df[[
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", metavar='metacelltypes.cts_final.txt', required=True,
                        help="celltypes called for aggregated metacells from celltyping.R (rule celltyping)")
    parser.add_argument("-a", metavar="metacell_assignment.tsv", required=True,
                        help="table with cell to metacell assignment from metacell_run_[method].py")
    parser.add_argument("-c", metavar="celltype_config.tsv", required=True,
                        help="table with major celltype to subcelltype assignment")
    parser.add_argument("-o", metavar="output_dir",
                        required=True, help="output_directory")
    parser.add_argument("-p", metavar="prefix", required=True,
                        help="prefix used for output files")
    parser.add_argument("-r", metavar="path/to/reportfile.rst",
                        help="report is written to this file", required=False, default=None)
    ...
    main(celltype_file, metacelltype_file, assignment_file,
         celltype_config_file, out_dir, out_prefix, reportfile)
