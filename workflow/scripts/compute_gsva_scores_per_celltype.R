...
# Anne Bertolini
# Jan 2024
# Modified by Matthias Lienhard, Aug 2024:
# * added snakemake opt parsing 
...
if (exists("snakemake")) {
  ...
  opt=list()
  opt$SCE <- snakemake@input[[1]]
  opt$subset_celltype <- snakemake@params[["tumor_celltypes"]]
  opt$outdir <- snakemake@params[["outdir"]]
  opt$geneset <- snakemake@params[["genesets"]]
  opt$sample <- snakemake@params[["sampleName"]]
  opt$min_set_size <- snakemake@params[["min_set_size"]]
  opt$method <- snakemake@params[["method"]]
  ...
  # Use optparse for standalone execution
  library(optparse)
...
my_sce <- my_sce[, mask_celltype, drop=FALSE]
