...
def main(h5file, out_dir, out_prefix, celltype_mapping_file, n_SEACells, n_pc, n_waypoint_eigs, n_topvar_genes, n_neighbors, min_iter, max_iter):
    ...
    model = fit_seacells_model(adata=adata, n_SEACells=n_SEACells,
                               n_waypoint_eigs=n_waypoint_eigs, min_iter=min_iter, max_iter=max_iter)
    # do some QC plots:
    # Check for convergence
    model.plot_convergence(
        save_as=f'{out_dir}/{out_prefix}_seacells_model_convergence.png')
    ...
    if celltype_mapping_file is not None:
        ...
        plt.close()
    SEACells.plot.plot_2D(adata, key='X_umap', show=False, colour_metacells=False,
                          save_as=f'{out_dir}/{out_prefix}_seacells_umap.png')
    SEACells.plot.plot_2D(adata, key='X_umap', show=False, colour_metacells=True,
                          save_as=f'{out_dir}/{out_prefix}_seacells_umap_color.png')
    SEACells.plot.plot_SEACell_sizes(
        adata, bins=5, show=False, save_as=f'{out_dir}/{out_prefix}_seacells_sizes.png')
    # export the assignments
    hard_df = get_assignment(model, hard=True)
    hard_df.to_csv(
        f'{out_dir}/{out_prefix}_seacells_hard_assignment.tsv', sep='\t')
    ...
    soft_df['fractionMT'] = adata.obs['fractionMT'].reindex(
        soft_df.index).values
    ...
    SEACell_ad = prepare_seacells(
        SEACell_ad,  n_topvar_genes, n_pc, n_neighbors)
    ...
    gene_dict = dict(adata.var["gene_names"])
...
def prepare_seacells(adata, n_topvar_genes, n_pc, n_neighbors):
    ...
    # Use the number of PCs computed earlier
    sc.pp.neighbors(adata, n_neighbors=n_neighbors, n_pcs=n_pc)
...
def get_assignment(model, hard=True):
    ...
    if hard:
        ...
        table = []
        metacell_dict = {bc: f'SEACell-{i}' for i,
                         bc in enumerate(archetype_labels)}
...
def export_seacells(seacells_obj, gene_dict, h5_out=None):
    ...
    if h5_out is not None:
        with h5py.File(h5_out, 'w') as f:
            # Save counts
            f.create_dataset(
                'raw_counts', data=seacells_obj.layers['raw'].toarray())
            ...
            # Save gene names
            gene_names = np.array([gene_dict[gid]
                                  for gid in seacells_obj.var_names], dtype='S')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Running SEACells metacells identification")
    parser.add_argument("-i", metavar='in_file.h5',
                        required=True, help="Input file with raw counts")
    parser.add_argument("-o", metavar='out_dir',
                        required=True, help="Output directory")
    parser.add_argument("-p", metavar="out_prefix",
                        required=True, help="Prefix for the output files")
    parser.add_argument("-c", metavar="celltype_mapping",
                        help="Optional: File with barcode celltype mappings. Used for celltype purity of metacells.", default=None)
    parser.add_argument("--n_cells", type=int, default=90,
                        help="Number of metacells [default: 90]")
    parser.add_argument("--n_pc", type=int, default=50,
                        help="PCA dimensions [default: 50]")
    parser.add_argument("--n_waypoint_eigs", type=int, default=10,
                        help="Number of eigenvalues to consider when initializing metacells [default: 10]")
    parser.add_argument("--n_topvar_genes", type=int, default=1500,
                        help="Number of top variable genes [default: 1500]")
    parser.add_argument("--n_neighbors", type=int, default=15,
                        help="Number of neighbors considered for the neighborhood graph [default: 15]")
    parser.add_argument("--iter", type=str, default="10,50",
                        help="Number of iterations to fit, provided as min,max [default: 10,50]")
    ...
    main(h5file, out_dir, out_prefix, celltype_mapping_file, n_SEACells,
         n_pc, n_waypoint_eigs, n_topvar_genes, n_neighbors, min_iter, max_iter)
