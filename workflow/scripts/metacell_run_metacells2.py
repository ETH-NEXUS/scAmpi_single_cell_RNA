...
def main(h5file, out_dir, out_prefix, celltype_mapping_file, target_metacell_umis, target_metacell_size, exclude_names, exclude_pattern, lateral_names, lateral_pattern):
    ...
    mc.pl.mark_lateral_genes(
        cells, lateral_gene_names=lateral_names, lateral_gene_patterns=lateral_pattern)
    ...
    metacells = mc.pl.collect_metacells(
        cells, name=f"{sample}.one-pass.metacells", random_seed=42)
    print(f'found {metacells.shape[0]} metacells')
    assign_df = get_assignment(cells)

    assign_df.to_csv(
        f'{out_dir}/{out_prefix}_metacells2_assignment.tsv', sep='\t')
    assign_df['UMI_counts'] = cells.X.sum(axis=1).A1
    assign_df['fractionMT'] = cells.obs['fractionMT'].reindex(
        assign_df['cell_barcode']).values
...
def aggregate_metacell_counts(assign_df, cells):
    ...
    aggregated_counts = [csr_matrix(
        cells.X[cell_metacell_ids == mid].sum(0)) for mid in metacell_ids]
    aggregated_counts = vstack(aggregated_counts, format='csr')
    metacell_obs = pd.DataFrame(
        {'total_umis': aggregated_counts.sum(1).A1}, index=metacell_ids)
    metacell_var = cells.var['gene_ids']
    aggregated_metacells = AnnData(
        X=aggregated_counts, obs=metacell_obs, var=metacell_var)
...
def h5_to_ad(h5file, celltype_mapping_file=None):
    ...
    adata = AnnData(X=counts, obs=pd.DataFrame(index=cell_names),
...
def export_metacells(metacells, h5_out=None):
    ...
    if h5_out is not None:
        with h5py.File(h5_out, 'w') as f:
            # Save counts
            f.create_dataset(
                'raw_counts', data=metacells.layers['total_umis'], dtype=int)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_metacell_umis", type=int,
                        default=100000, help="Target number of umis per metacell")
    parser.add_argument("--target_metacell_size", type=int,
                        default=30, help="Target number of cells per metacell")
    parser.add_argument("--exclude_names", type=str, default=','.join(EXCLUDED_GENE_NAMES),
                        help=", seperated list of genes to exclude (sex, non-coding, mt, ...)")
    parser.add_argument("--exclude_pattern", type=str,
                        default=EXCLUDED_GENE_PATTERNS[0], help=", seperated list of pattern to exclude (sex, non-coding, mt, ...)")
    parser.add_argument("--lateral_names", type=str, default=','.join(EXCLUDED_GENE_NAMES),
                        help=", seperated list of genes to mask as lateral (cell cycle, stress response, ribosomal, ...)")
    parser.add_argument("--lateral_pattern", type=str,
                        default=EXCLUDED_GENE_PATTERNS[0], help=", seperated list of pattern to mask as lateral (cell cycle, stress response, ribosomal ...)")
    ...
    main(h5file, out_dir, out_prefix, celltype_mapping_file, target_umis,
         target_size, exclude_names, exclude_pattern, lateral_names, lateral_pattern)
