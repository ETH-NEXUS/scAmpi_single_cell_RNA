...
for(group in seq(length(gene.list.all))){
  ...
  if(nchar(paste(genes.not.expressed[1:5],collapse="")) > 32 ){split_by <- 3}
  else{split_by <- 5}
...
p_bar = ggplot(t.plot, aes(x=x, y=value, fill=Celltypes)) + xlab("") + ylab("Percent") +
