...
vst <- function(umi,
  ...
  if (!is.null(latent_var_nonreg)) {
    if (verbosity > 0) {
      message('Estimating parameters for following non-regularized variables: ', paste(latent_var_nonreg, collapse=', '))
    }
    if (!is.null(batch_var)) {
    ...
    for (i in 1:max_bin) {
      ...
      if (min_variance == "model_mean") {
        ...
        res[genes_bin, ] <- switch(residual_type,
                                   'pearson' = sctransform:::pearson_residual2(y, mu, model_pars_final[genes_bin, 'theta'], min_vars = mu_mean_var),
                                   'deviance' = sctransform:::deviance_residual(y, mu, model_pars_final[genes_bin, 'theta']),
        ...
        res[genes_bin, ] <- switch(residual_type,
                                   'pearson' = sctransform:::pearson_residual2(y, mu, model_pars_final[genes_bin, 'theta'], min_vars = mu_median_var),
                                   'deviance' = sctransform:::deviance_residual(y, mu, model_pars_final[genes_bin, 'theta']),
                                   stop('residual_type ', residual_type, ' unknown - only pearson and deviance supported at the moment'))
      } else {
        res[genes_bin, ] <- switch(residual_type,
                                   'pearson' = sctransform:::pearson_residual(y, mu, model_pars_final[genes_bin, 'theta'], min_var = min_var),
                                   'deviance' = sctransform:::deviance_residual(y, mu, model_pars_final[genes_bin, 'theta']),
                                   stop('residual_type ', residual_type, ' unknown - only pearson and deviance supported at the moment')
        )
        }
...
get_model_pars <- function(genes_step1, bin_size, umi, model_str, cells_step1,
  ...
  if (fix_slope | fix_intercept) {
    ...
    if (use_geometric_mean_offset){
      gene_gmean <- stransform:::row_gmean(umi)
      model_pars_fixed <- cbind(rep(NA, length(genes_step1)),
  ...
  if (startsWith(x = method, prefix = 'offset')) {
    ...
    if (method == 'offset_shared_theta_estimate') {
      ...
      if (sum(use_genes) > 250) {
        o <- order(-sctransform:::row_gmean(umi[use_genes, ]))
  ...
  for (i in 1:max_bin) {
    ...
    par_lst <- future_lapply(
      ...
      FUN = function(indices) {
        ...
        if (method == 'poisson') {
          return(sctransform:::fit_poisson(umi = umi_bin_worker, model_str = model_str, data = data_step1, theta_estimation_fun = theta_estimation_fun))
        }
        else if (method == 'qpoisson') {
          return(sctransform:::fit_qpoisson(umi = umi_bin_worker, model_str = model_str, data = data_step1))
        }
        else if (method == 'nb_theta_given') {
          theta_given_bin_worker <- theta_given_bin[indices]
          return(sctransform:::fit_nb_theta_given(umi = umi_bin_worker, model_str = model_str, data = data_step1, theta_given = theta_given_bin_worker))
        }
        else if (method == 'nb_fast') {
          return(sctransform:::fit_nb_fast(umi = umi_bin_worker, model_str = model_str, data = data_step1, theta_estimation_fun = theta_estimation_fun))
        }
        else if (method == 'nb') {
          return(sctransform:::fit_nb(umi = umi_bin_worker, model_str = model_str, data = data_step1))
        }
...
get_model_pars_nonreg <- function(genes, bin_size, model_pars_fit, regressor_data, umi, model_str_nonreg, cell_attr, verbosity) {
  cat('entering patched version of get_model_pars_nonreg')
  bin_ind <- ceiling(x = 1:length(x = genes) / bin_size)
  max_bin <- max(bin_ind)
  if (verbosity > 1) {
    pb <- txtProgressBar(min = 0, max = max_bin, style = 3)
  }
  model_pars_nonreg_list <- list()
  var_names=all.vars(as.formula(model_str_nonreg))
  var_names[1]='(Intercept)'
  default_coef=setNames(rep(0,length(var_names)),var_names)
  for (i in 1:max_bin) {
    genes_bin <- genes[bin_ind == i]
    mu <- tcrossprod(model_pars_fit[genes_bin, -1, drop=FALSE], regressor_data)
    umi_bin <- as.matrix(umi[genes_bin, ])
    model_pars_nonreg_list[[i]] <-
  ...
  # unlist the bin lists
  model_pars_nonreg_list = unlist(model_pars_nonreg_list, recursive=FALSE)
  #handle the warnings
  warn_vec <- sapply(model_pars_nonreg_list, function(lst) lst[['warning']])
  
  if (any(warn_vec != '')){
    # genes with warnings
    warning_genes=paste(genes[warn_vec!= ''], collapse=', ')
    # warning messages
    tbl=table(warn_vec[warn_vec != ''])
    warning_msgs=paste0(tbl, " times:\n", names(tbl), "\n")
    msg=paste0('glm() rose warnings for ',sum(warn_vec != ''),' genes:\n',
        warning_genes,'\n',
        'Messages:\n',
        paste(warning_msgs, collapse=''),'\n')
    cat(msg)
  }
  cat('concatenate results to array\n')
  model_pars_nonreg <- t(sapply(model_pars_nonreg_list, function(lst) lst[['coefficients']], simplify=TRUE))
  tryCatch(
  rownames(model_pars_nonreg) <- genes
  , 
  error=function(e){
    cat(length(genes), ' --- ', dim(model_pars_nonreg),'\n\nnames:')
    cat(unique(sapply(model_pars_nonreg, function(x) paste(names(x)))))
    cat('/n')
  })
...
reg_model_pars <- function(model_pars, genes_log_gmean_step1, genes_log_gmean, cell_attr,
  ...
  if (exclude_poisson | fix_slope | fix_intercept){
    ...
    if (is.null(genes_var)) genes_var <- sctransform:::row_var(umi)
  ...
  if (is.null(batch_var)){
    ...
    for (b in batches) {
      ...
      if (use_geometric_mean){
        batch_genes_log_gmean_step1 <- log10(sctransform:::row_gmean(umi[genes_step1, sel], eps = gmean_eps))
      } else {
        batch_genes_log_gmean_step1 <- log10(rowMeans(umi[genes_step1, sel]))
      }
      ...
      if (use_geometric_mean){
        batch_genes_log_gmean <- log10(sctransform:::row_gmean(umi[, sel], eps = gmean_eps))
      } else {
        batch_genes_log_gmean <- log10(rowMeans(umi[, sel]))
      }
  ...
  if (fix_intercept){
    ...
    if (use_geometric_mean_offset){
      gene_gmean <- sctransform:::row_gmean(umi)
