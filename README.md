...
- **raw FASTQ input**  
  ...
  **NOTE:** SAMPLENAME can only contain the following characters [a-zA-Z0-9_-]+  
  ...
  ```
In order to facilitate working around this inflexibility, there is a rule to create this folder structure and file names. This requires to provide an additional column in the sample map `file_stem`, which corresponds to the common part of the file name, and may contain special characters.  The fastq files starting with this file stem are assumed to correspond to one sample and links to Cell Ranger compatible folder structure are created automatically. 
...
```
### MetaCells2 
Due to the output of metacells2, as well as the suggested manual filtering procedure, this algorithm is not suggested in the context of scAmpi, but implemented for testing. 
